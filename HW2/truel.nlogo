...
to go
  ...
  ][                           ;; if play is sequential, players shoot and die sequentially (specified order)
    foreach [ 0 1 2] [
      ask turtle-set player ? [shoot]  ;; "turtle-set" command used here so asking dead players to shoot doesn't stop the simulation
      check-die
    ]
  ]
end
...
to shoot                              ;; routine for players to pick a target (rationally) and shoot
  ...
  [
    if random-float 1 < accuracy  ;; players have same accuracy regardless of target, so check if shot hits first
    [
      ifelse who = 0                  ;; Player 1 has special shoot command because they are aware of Player 2's ammunition
        [let target max-one-of other players with [ammo > 0] [accuracy]
         ifelse target != nobody ;; Player 1 targets other player with highest accuracy, as long as they have bullets
           [ask target [
               set alive? false
        ...
        [ask turtle-set max-one-of other players [accuracy] ;; Players 2 and 3 target other player with the highest accuracy
          [set alive? false
            create-link-from myself]]
    ]
...
to check-die  ;; routine to kill players that have been shot and to monitor Player 1's payoff
    ...
    if diag? = false [ask players with [alive? = false] [ die ]]      ;; kill players that have been shot
...
@#$#@#$#@
<experiments>
  <experiment name="simultaneous" repetitions="10000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>count players &lt;= 1</exitCondition>
    <metric>payoff</metric>
    <steppedValueSet variable="bullets" first="0" step="1" last="5"/>
    <enumeratedValueSet variable="q">
      <value value="0.5"/>
      <value value="0.95"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="sequential?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="diag?">
      <value value="false"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="sequential" repetitions="10000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>count players &lt;= 1</exitCondition>
    <metric>payoff</metric>
    <metric>player 0 != nobody</metric>
    <steppedValueSet variable="bullets" first="0" step="1" last="5"/>
    <enumeratedValueSet variable="q">
      <value value="0.5"/>
    </enumeratedValueSet>
